...
import { matchs } from "@public/data/matches";
            ...
            <div className="4xl:col-start-2 4xl:col-end-12">
              <div className="flex-y justify-between flex-wrap gap-24p mb-30p">
                <h3 className="heading-3 text-w-neutral-1">
                  Total Participants ({matchs.length})
                </h3>
              ...
              <div className="grid xl:grid-cols-1 md:grid-cols-2 grid-cols-1 justify-center max-xl:gap-x-30p divide-y divide-shap/70 4xl:px-[140px] px-80p px-120p py-80p bg-b-neutral-3">
                {matchs.map((item, idx) => (
                  <div
                    key={idx}
                    className="relative flex max-xl:flex-col items-center justify-center gap-y-30p gap-x-40p sm:py-6 py-5"
                  >
                    <div className="flex items-center gap-3 min-w-[180px]">
                      <Link
                        href={`/profile/${idx}`}
                        className="text-m-medium text-w-neutral-1 line-clamp-1 link-1"
                      >
                  ...
                  <div className="relative flex max-sm:flex-col items-center justify-center gap-y-30p gap-x-40p sm:pb-6 pb-5 mb-60p">
                    <div className="flex items-center gap-3 min- w-[180px]">
                      <div>
                        ...
                        >
                          {matchs[openModal]?.leftPlayer?.name}
                        </Link>
                        <Link
                          href="/teams/1"
                      ...
                      <Image
                        className="avatar size-48p shrink-0"
                        src={matchs[openModal]?.leftPlayer?.image}
                        width={48}
                        height={48}
                        alt="user"
                      />
                    ...
                    <div className="flex items-center gap-3 min- w-[180px]">
                      <div>
                        <Link
                          ...
                          className="text-m-medium text-w-neutral-1 line-clamp-1 link-1 mb-1"
                        >
                          {matchs[openModal]?.rightPlayer?.name}
                        </Link>
                        <Link
                          href="#"
                          className="text-xs-medium text-secondary"
                      ...
                      <Image
                        ...
                        width={48}
                        height={48}
                        src={matchs[openModal]?.rightPlayer?.image}
                        alt="user"
                      />
                    </div>
                  </div>
