...
import PostModal from "@/components/sections/preference/postmodal";

type AllBlogsProps = {
  mode: "hot" | "recent";
};

const Postcard = ({ mode }: AllBlogsProps) => {
      ...
      <div className="container">
        <div className="grid 3xl:grid-cols-4 xl:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-30p">
          {filteredPosts.map((item, idx) => (
            <div
              ...
              className="bg-b-neutral-3 py-24p px-30p rounded-12 group cursor-pointer"
            >
              ...
              <div className="heading-5 text-w-neutral-1 leading-[130%] line-clamp-2 link-1">
                {item.title}
              </div>
            </div>
          ))}
        ...
        {selectedPost && (
                <PostModal onClose={() => setSelectedPost(null)}>
                    ...
                    />
                    <p className="text-gray-700">{selectedPost.content || "내용 준비 중..."}</p>
                </PostModal>
                )}
