...
const Modal = ({ children, style, open, onClick }: Props) => {
  ...
  return (
    <div
      onClick={onClick}
      className={`fixed inset-0 z-[500] min-h-screen w-screen bg-black/50 backdrop-blur-sm ${
        open ? "translate-y-0" : "-translate-y-full"
      }`}
    >
      <div
        className={`absolute left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2 duration ${
          open ? "visible opacity scale" : "invisible scale-75 opacity-0"
        }
      ...
      >
        <div className="relative w-full h-full">
          ...
          >
            <i className={`${style} md:text-base sm:text-sm text-[8px]`} />
          </button>
